...
class SearchActivity : CoreActivity<ActivitySearchBinding>() {
  ...
  private val jsonPickerLauncher =
    registerForActivityResult(JsonFilePicker()) { uri ->
      uri?.let {
  ...
  override fun setListeners(isEnabled: Boolean) {
    if (isEnabled) {
      ...
      adapter.itemClickListener = CustomClickListener { position, _, _ ->
        showInterstitialAd { copyToClipboard("API Key", adapter.items[position].key) }
      }
  ...
  private fun showAddKeyDialog() {
    ...
    MaterialAlertDialogBuilder(this).setView(dialogBinding.root).create().also { dialog ->
      dialogBinding.btnCancel.setOnClickListener { dialog.dismiss() }
      dialogBinding.btnAdd.setOnClickListener {
        val appName = dialogBinding.inputAppName.text?.toString()?.trim().orEmpty()
        val apiKey = dialogBinding.inputApiKey.text?.toString()?.trim().orEmpty()
        if (appName.isNotEmpty() && apiKey.isNotEmpty()) {
          viewModel.addKey(appName, apiKey)
          dialog.dismiss()
        } else {
          if (appName.isEmpty()) {
            dialogBinding.inputAppNameLayout.error = getString(R.string.key_app_name)
          }
          if (apiKey.isEmpty()) {
  ...
  private fun setupUI() {
    ...
    launch {
      SongDetailDataUtil.getInstance().getCountLive(SongRetrievalMode.NOT_FOUND).collectLatest {
        withContext(mainDispatcher) {
          binding.txtSongsRemainToBeFound.text = getString(R.string.songs_remain_to_be_found, it)
        }
      }
    }
    launch {
