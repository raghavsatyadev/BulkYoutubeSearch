...
class FoundSongsHolder(
  ...
  init {
    binding.btnDelete.setOnClickListener {
      itemClickListener?.onItemClick(layoutPosition, binding.btnDelete, false)
    }
  }
  ...
  override fun bind(model: SongDetail, itemViewType: Int, position: Int, itemCount: Int) {
    with(binding) {
      with(model) {
        txtFoundSongTitle.text = Html.fromHtml(title, Html.FROM_HTML_MODE_COMPACT).toString()
        txtFoundSongArtist.text = Html.fromHtml(artist, Html.FROM_HTML_MODE_COMPACT).toString()
        imgFoundSong.loadImage(
          thumbnailLow,
          placeHolder = placeholder,
          crossFade = true,
          listener = { imgFoundSong.loadImage(thumbnailHigh, crossFade = true) },
        )
      }
    }
  }
