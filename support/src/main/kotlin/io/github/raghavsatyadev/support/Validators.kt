...
object Validators {
  ...
  /**
   * Checks for 1 Letter between A-E, a hyphen, 1-2 digits between 1-13, a zero and 1 digit between
   * 1-4
   */
  fun isValidFlatNumber(flatNumber: String?): Boolean {
    val pattern = "^[A-E]-([1-9]|1[0-3])0[1-4]$"
    return !TextUtils.isEmpty(flatNumber) &&
      Pattern.compile(pattern.trim()).matcher(flatNumber!!).matches()
  }
  ...
  fun isValidNumberPlate(numberPlate: String): Boolean {
    ...
    return !TextUtils.isEmpty(numberPlate) &&
      Pattern.compile(pattern.trim()).matcher(numberPlate).matches()
  }
}
