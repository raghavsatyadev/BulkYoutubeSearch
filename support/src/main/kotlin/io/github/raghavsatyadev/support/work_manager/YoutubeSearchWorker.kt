...
class YoutubeSearchWorker(appContext: Context, workerParams: WorkerParameters) :
  ...
  companion object {
    ...
    private val manager by lazy { WorkManager.getInstance(CoreApp.instance) }
    ...
    suspend fun startWorker() {
      ...
      val periodicWorkInfos =
        withContext(Dispatchers.IO) {
          manager.getWorkInfosForUniqueWork(UNIQUE_WORK_NAME_PERIODIC).get()
        }
        ...
        oneTimeWorkStatus.collectLatest { infos ->
          ...
          if (!oneTimeWorkRunning) {
            ...
            val periodicWorkRequest =
              PeriodicWorkRequestBuilder<YoutubeSearchWorker>(1, TimeUnit.DAYS).build()
